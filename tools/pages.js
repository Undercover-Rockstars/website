...
${PRODUCTS.map((p, i) => `          <button type="button" data-fit-product="${p.id}" aria-pressed="${i === 0}"><span>UR/${p.code} ${esc(p.name)}</span><span class="p">${format(p.price)}</span></button>`).join('\n')}
  ...
  <section class="section" id="fit-viewer">
    <div class="grid12" style="margin-bottom:40px">
      <p class="eyebrow col-1-3">UR/V · the fit viewer</p>
      <div class="col-4-9">
        <h2 class="h-display" style="margin-bottom:20px">See where<br><span class="acc">the cut sits.</span></h2>
        <p class="lede" style="max-width:58ch">A body generated from your own measurements, with the garment's ease drawn as a shell around it. The gap between the two is the fit, and it is what made to measure buys. Generated from your measurements: not a photograph of the garment, not a scan of you, and nothing your profile does not contain is drawn, because nothing beyond it was measured.</p>
      </div>
    </div>
    <div class="code" id="viewer-panel">
      <div class="vw-controls">
        <div class="vw-chips" role="group" aria-label="Day or night version of the cut">
          <button type="button" data-vw-mode="day">Day cut</button>
          <button type="button" data-vw-mode="night" aria-pressed="true">Night cut</button>
        </div>
        <div class="vw-chips" role="group" aria-label="Camera view">
          <button type="button" data-vw-preset="full" aria-pressed="true">Full</button>
          <button type="button" data-vw-preset="torso" aria-pressed="false">Torso</button>
          <button type="button" data-vw-preset="detail" aria-pressed="false">Detail</button>
        </div>
        <div class="vw-chips" role="group" aria-label="Standard size for the size-run body" id="vw-sizes" hidden>
${SIZES.map(z => `          <button type="button" data-vw-size="${z}"${z === 'M' ? ' aria-pressed="true"' : ' aria-pressed="false"'}>${z}</button>`).join('\n')}
        </div>
      </div>
      <div class="vw-stage" id="vw-stage" hidden></div>
      <div class="cap-controls" id="vw-open-row">
        <button type="button" class="btn" id="vw-open"><span>Open the fit viewer</span><span aria-hidden="true">→</span></button>
        <button type="button" class="btn btn--ghost" id="vw-reload" hidden><span>Use my saved measurements</span><span aria-hidden="true">↺</span></button>
      </div>
      <p class="fit-note" id="vw-status">The viewer loads only when you open it, about 730 KB once, then cached like the scanner. Nothing about your profile leaves this browser to draw it.</p>
      <p class="fit-note" id="vw-source"></p>
      <p class="fit-note" id="vw-ease"></p>
      <p class="fit-note">Drag to turn, pinch or scroll to zoom; left alone it turns slowly by itself. Day and night cuts share every seam of the geometry, like the garments themselves: the switch changes colour and finish only. If no profile is saved here, the viewer shows the standard size-run body for the size you pick, labelled as a standard size, not you.</p>
    </div>
  </section>
</main>
...
/* The service worker lives at /fit/sw.js so its scope is /fit/ and nothing
   ...
   /assets/vendor/mediapipe/) and the viewer's three.js (under
   /assets/vendor/three/) are deliberately NOT precached: they are heavy,
   they load only after consent and Begin / after the buyer opens the
   viewer, and the worker's stale-while-revalidate caches them on demand,
   so the first visit stays light and the second works offline. */
...
The same page carries the **fit viewer**, a 3D view inside the app: a
generated body lofted from the buyer's own saved measurements (elliptical
rings at the chest, waist and seat, limbs along the measured lengths,
everything unmeasured closed off plainly) with the garment drawn as a
translucent shell around it, so the ease the category's cut adds at each
level is the visible thing. It is **not a render of the garment and not
photography**: there are no garment models and no fabric in it, and it
cannot show any shape the profile does not contain. Day and night versions
share geometry and differ by colour only, driven by the site's mode switch.
With no profile saved it shows the standard size-run body for the chosen
size, labelled as a standard size rather than the buyer. The block and ease
numbers behind it (and behind the "cut from an M, sleeve +2 cm" sentence on
the profile screen and in reservation emails) are a provisional draft from
standard grading, awaiting the pattern cutter's real numbers. Say all of
this if you describe the viewer; do not call it a try-on, a preview of the
product, or photorealistic.
...
write('catalogue.json', JSON.stringify({
  ...
  fittingRoom: { url: ORIGIN + '/fit/', app: 'UR Fit', measurementConnected: true,
    ...
    viewer: {
      url: ORIGIN + '/fit/#fit-viewer',
      generated: true,
      what: 'a generated body lofted from the buyer\u2019s own saved measurements, with the garment\u2019s drafted ease drawn as a translucent shell around it',
      notGarmentRender: true,
      notPhotography: true,
      notAScanOfYou: true,
      fallbackWithoutProfile: 'the standard size-run body for the chosen size, labelled as a standard size',
      blockData: 'provisional draft from standard grading in assets/ur-data.js (SIZE_RUN, BLOCKS, blockFor), awaiting the pattern cutter\u2019s real numbers',
      onScreenLabel: 'generated from your measurements, not a photograph of the garment'
    } },
